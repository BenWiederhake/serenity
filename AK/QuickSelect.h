...
static inline int random_int(size_t min, size_t max)
{
...
size_t median_of_medians(Collection& collection, size_t left, size_t right, LessThan less_than)
{
    ...
    // We're using mutual recursion here, using quick_select to find the pivot for quick_select.
...
template<typename Collection, typename PivotFn, typename LessThan>
size_t quick_select(Collection& collection, size_t left, size_t right, size_t k, PivotFn pivot_fn, LessThan less_than)
{
    ...
    if (k < pivot_index)
        return quick_select(collection, left, pivot_index - 1, k, pivot_fn, less_than);

    // recurse on the right side
    return quick_select(collection, pivot_index + 1, right, k, pivot_fn, less_than);
}

//
template<typename Collection, typename PivotFn, typename LessThan>
size_t quick_select(Collection& collection, size_t k, PivotFn pivot_fn, LessThan less_than)
{
    return quick_select(collection, 0, collection.size() - 1, k, pivot_fn, less_than);
}

template<typename Collection, typename PivotFn>
size_t quick_select(Collection& collection, size_t k, PivotFn pivot_fn)
{
    return quick_select(collection, 0, collection.size() - 1, k, pivot_fn, [](auto& a, auto& b) { return a < b; });
}
...
size_t quick_select(Collection& collection, size_t k)
{
