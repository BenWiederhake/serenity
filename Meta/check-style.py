...
def is_forbidden(filename, forbidden_list):
    return any(
        filename.startswith(forbidden_prefix) for forbidden_prefix in forbidden_list
    )
...
def run():
    ...
    for filename in find_files_here_or_argv():
        with open(filename, "r") as f:
            file_content = f.read()
        if not is_forbidden(filename, LICENSE_HEADER_CHECK_EXCLUDES):
            if not GOOD_LICENSE_HEADER_PATTERN.search(file_content):
                errors_license.append(filename)
        if filename.endswith('.h'):
            if is_forbidden(filename, PRAGMA_ONCE_CHECK_EXCLUDES):
                # File was excluded
                pass
        ...
        if not is_forbidden(filename, LIBC_CHECK_EXCLUDES):
            if BAD_INCLUDE_LIBC.search(file_content):
                errors_include_libc.append(filename)
        if not is_forbidden(filename, INCLUDE_CHECK_EXCLUDES):
