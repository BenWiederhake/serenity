...
def get_files_here():
    result = subprocess.run(['git', 'ls-files', '--', '*.cpp', '*.h', ':!:Ladybird/*'], check=True, capture_output=True, text=True)
...
def report_inclusion_stats(deps):
    compiled_files = 0
    targets = 0
    for compilation_target in deps.keys():
        if not compilation_target.endswith(".cpp"):
            continue
        # "Whatever-First-Search". Since the size/depth of the tree is hopefully small enough, this shouldn't matter too much.
        closed_set = set()
        open_set = {compilation_target}
        while open_set:
            current_file = open_set.pop()
            assert current_file not in closed_set
            closed_set.add(current_file)
            direct_deps = deps[current_file]
            new_deps = set(direct_deps).difference(closed_set)
            open_set.update(new_deps)
        compiled_files += len(closed_set)
        targets += 1
    print(f"// {targets} compilation targets will read {compiled_files} files, not counting unresolved includes.")
...
def run(arch):
    ...
    headers_list = get_files_here()
    if SHOW_TIMING:
        time_listed = time.time_ns()
    if USE_ONLY_GIT:
        deps = {header_name: get_dependencies(arch, header_name) for header_name in headers_list}
    else:
        missing_headers = set(headers_list)
        assert len(missing_headers) == len(headers_list), "Duplicate git enumeration?!"
        deps = dict()
        while True:
            deps.update({header_name: get_dependencies(arch, header_name) for header_name in missing_headers})
            missing_headers = set(single_dep for direct_deps in deps.values() for single_dep in direct_deps).difference(set(deps.keys()))
            if not missing_headers:
                break
            print(f"// Analyzed {len(deps)} files, still missing at least {len(missing_headers)} files ...")
    with open(HEADER_NETWORK_JSON, "w") as fp:
        json.dump(deps, fp, indent=1)
    if SHOW_TIMING:
    ...
    print(f'These cycles can be split into {len(nontrivial_connected_components)} components. (Higher ratio to number of cycles is better.)',
          file=sys.stderr)
...
def guess_architecture():
    matches = []
    for arch in 'aarch64 x86_64 lagom'.split():
